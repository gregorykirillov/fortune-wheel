...
try {
  ...
  if (error?.code === "ENOENT") {
    console.error(
      "Файл variants.json по умолчанию должен находиться рядом с server.mjs"
    );
  }
...
try {
  ...
  console.error(
    "Используется невалидный формат данных (по умолчанию json) или json сломан"
  );
...
server.listen(5000, () => {
  console.info(`Server start on http://localhost:5000/`);
});
